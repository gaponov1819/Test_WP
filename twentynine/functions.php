...
function true_options() {
  global $true_page;
  add_options_page( 'Настройка курса евро', 'Настройка курса евро', 'manage_options', $true_page, 'true_option_page');  
}
...
function true_option_settings() {
  ...
  add_settings_section( 'true_section_1', 'Курс евро', '', $true_page );
  ...
  add_settings_field( 'change', 'Введите актуальный курс евро', 'true_option_display_settings', $true_page, 'true_section_1', $true_field_params );
}
...
function true_option_display_settings($args) {
  ...
  switch ($type) {  
    case 'text':  
      ...
      echo "<input class='regular-text' required type='text' id='$id' name='" . $option_name . "[$id]' value='$o[$id]' />";  
      echo ($desc != '') ? "<br /><span class='description'>$desc</span>" : "";  
    break; 
  }
}

//Замена знака гривны на евро
add_filter('woocommerce_currency_symbol', 'change_symbol', 9999, 2);
 
function change_symbol( $value_symbol, $value_code ) {
	if( $value_code === 'UAH' ) {
		return '€';
	}
	return $value_symbol;
...
//Конвертация в евро согласно курса
add_filter( 'woocommerce_variable_sale_price_html', 'wc_wc20_variation_price_format', 10, 2 );
add_filter( 'woocommerce_variable_price_html', 'wc_wc20_variation_price_format', 10, 2 );
function wc_wc20_variation_price_format( $price, $product ) {
	$options = get_option('true_options');
	$prices = array( $product->get_variation_price( 'min', true ), $product->get_variation_price( 'max', true ) );
	$price = wc_price( $prices[0]/$options['kurs_evro'] ).'-'.wc_price( $prices[1]/$options['kurs_evro'] );
return $price;
	}